...
el.dropZone.addEventListener("click",e=>{if(e.target!==el.chooseVideoButton)openFilePicker(e)});
el.chooseVideoButton.addEventListener("click",openFilePicker);
el.dropZone.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key===" ")openFilePicker(e)});
el.videoFile.addEventListener("change",()=>{const f=el.videoFile.files?.[0];if(f)file(f)});
